...
# Compiled at: 2023-06-08 13:52:37
...
from ableton.v3.control_surface.elements.color import ColorPart, ComplexColor
...
def make_color_for_liveobj(obj, is_selected=False):
    if liveobj_valid(obj):
        if obj.color_index is not None:
            color = LIVE_COLOR_INDEX_TO_RGB.get(obj.color_index, 0)
            if is_selected:
                color = Rgb.WHITE
        return color
    return Rgb.OFF
...
class Rgb:
    ...
    WHITE_BLINK = SimpleColor(3, channel=(midi.BLINK_LED_CHANNEL))
